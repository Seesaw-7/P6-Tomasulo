...
static unsigned int avail_mem =
    sizeof(heap); // the most CONTIGUOUS memory available
...
static Header *getmoremem(unsigned int total_size) {
  if (avail_mem < total_size)
    return NULL; // gg, we ran out of mem :P
  Header *up = (Header *)next_index;
  next_index = (char *)(next_index) + total_size; // allocate the block
...
void *tj_malloc(unsigned int size) {
  ...
  unsigned int total_size =
      size + sizeof(Header); // need to include the header size as well
