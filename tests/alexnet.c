...
void fc_layer(int weights[5][5], int biasses[5], int inputs[5], int outputs[5],
              int input_num, int output_num, int af_num) {
  int i;
  int j;
...
void fc_weight_generator(int weights[5][5], int biasses[5], int input_num,
                         int output_num) {
  int i;
  int j;
  for (i = 0; i < output_num; i++) {
...
void cnn_layer(int weights[5][5][5][5], int biasses[5], int inputs[5][5][5],
               int outputs[5][5][5], int input_channel, int output_channel,
               int input_size, int kernel_size, int stride, int zero_pad,
               int af_num) {
  ...
  for (o_ch_itr = 0; o_ch_itr < output_channel; o_ch_itr++) {
    for (o_r_itr = 0; o_r_itr < output_size; o_r_itr++) {
      for (o_c_itr = 0; o_c_itr < output_size; o_c_itr++) {
          ...
          for (k_r_itr = 0; k_r_itr < kernel_size; k_r_itr++) {
            for (k_c_itr = 0; k_c_itr < kernel_size; k_c_itr++) {
              outputs[o_ch_itr][o_r_itr][o_c_itr] +=
                  (((stride * o_r_itr) + k_r_itr - zero_pad) < 0) ||
                          (((stride * o_c_itr) + k_c_itr - zero_pad) < 0) ||
                          (((stride * o_r_itr) + k_r_itr - zero_pad) >=
                           input_size) ||
                          (((stride * o_c_itr) + k_c_itr - zero_pad) >=
                           input_size)
                      ? 0
                      : inputs[i_ch_itr]
                              [(stride * o_r_itr) + k_r_itr - zero_pad]
        ...
        case 0:
          outputs[o_ch_itr][o_r_itr][o_c_itr] =
              relu_af(outputs[o_ch_itr][o_r_itr][o_c_itr]);
          break;
        case 1:
          outputs[o_ch_itr][o_r_itr][o_c_itr] =
              relu_af(outputs[o_ch_itr][o_r_itr][o_c_itr]);
...
void cnn_pool(int inputs[5][5][5], int outputs[5][5][5], int feature_channel,
              int input_size, int kernel_size, int stride, int zero_pad,
              int pool_num) {
  int output_size = ((input_size + (2 * zero_pad) - kernel_size) >> stride) + 1;
  int new_candidate;
  ...
  for (ch_itr = 0; ch_itr < feature_channel; ch_itr++) {
    for (o_r_itr = 0; o_r_itr < output_size; o_r_itr++) {
      for (o_c_itr = 0; o_c_itr < output_size; o_c_itr++) {
        switch (pool_num) {
        case 0:
          outputs[ch_itr][o_r_itr][o_c_itr] =
              inputs[ch_itr][stride * o_r_itr][stride * o_c_itr];
          break;
        case 1:
        ...
        for (k_r_itr = 0; k_r_itr < kernel_size; k_r_itr++) {
          for (k_c_itr = 0; k_c_itr < kernel_size; k_c_itr++) {
            new_candidate = inputs[ch_itr][(stride * o_r_itr) + k_r_itr]
                                  [(stride * o_c_itr) + k_c_itr];
            switch (pool_num) {
            case 0:
              outputs[ch_itr][o_r_itr][o_c_itr] =
        ...
        case 1:
          outputs[ch_itr][o_r_itr][o_c_itr] =
              outputs[ch_itr][o_r_itr][o_c_itr] - (kernel_size * kernel_size);
          break;
        }
      }
    }
...
void cnn_input_generator(int inputs[5][5][5], int input_channel,
                         int input_size) {
...
void cnn_weight_generator(int weights[5][5][5][5], int biasses[5],
...
void cnn_to_fc(int cnn_feature[5][5][5], int cnn_feature_channel,
               int cnn_feature_size, int fc_feature[5]) {
  ...
  for (ch_itr = 0; ch_itr < cnn_feature_channel; ch_itr++) {
    for (r_itr = 0; r_itr < cnn_feature_size; r_itr++) {
      for (c_itr = 0; c_itr < cnn_feature_size; c_itr++) {
        fc_feature[(ch_itr * cnn_feature_size * cnn_feature_size) +
                   (r_itr * cnn_feature_size) + c_itr] =
            cnn_feature[ch_itr][r_itr][c_itr];
      }
    }
  }
}
...
int main() {
  ...
  for (itr = 0; itr < TEST_ITR; itr++) {
    ...
    cnn_weight_generator(cnn_weights, cnn_biasses, cnn_input_channel,
                         cnn_output_channel, cnn_kernel_size);
    cnn_layer(cnn_weights, cnn_biasses, cnn_inputs, cnn_outputs,
              cnn_input_channel, cnn_output_channel, cnn_input_size,
              cnn_kernel_size, cnn_stride, cnn_zero_padd, cnn_af_type);
    ...
    cnn_pool(cnn_outputs, cnn_inputs, cnn_input_channel, cnn_input_size,
             cnn_kernel_size, cnn_stride, cnn_zero_padd, cnn_pool_type);
    ...
    fc_layer(fc_weights, fc_biasses, fc_inputs, fc_outputs, fc_input_num,
             fc_output_num, fc_af_type);
  }
